...
Proof.
  ...
  apply ts_mutind; intros; subst;
    repeat match goal with
           | [ H: _ = trm_path _  |- _] => inversions H
           end;
    try solve [eapply sel_replacement; auto]; eauto.
  - destruct* (sngl_replacement Hi
                                (H _ eq_refl eq_refl)
                                (H0 _ eq_refl eq_refl) r).
  - apply repl_swap in r.
    destruct* (sngl_replacement Hi
                                (H _ eq_refl eq_refl)
                                (H0 _ eq_refl eq_refl) r).
Qed.
...
Lemma general_to_tight_typing: forall G p T,
  inert G ->
  G ⊢ trm_path p : T ->
  G ⊢# p : T.
Proof.
...
Lemma general_to_tight_subtyping: forall G S T,
  inert G ->
  G ⊢ S <: T ->
  G ⊢# S <: T.
Proof.
...
Lemma general_to_tight_val : forall G v T,
    inert G ->
    G ⊢ (trm_val v) : T ->
    G ⊢#v v : T.
Proof.
  intros * Hin.
  remember (trm_val v) as t eqn:Heq.
  induction 1; inversions Heq;
    eauto using general_to_tight_subtyping.
Qed.
...
Ltac proof_recipe :=
  ...
  | [ Hg: ?G ⊢ _ : _,
      Hi: inert ?G |- _ ] =>
    try apply (general_to_tight_typing Hi) in Hg;
    try apply (general_to_tight_val Hi) in Hg;
