...
Require Import RecordTypes InertTypes DefRecordInertTyping PreciseTyping HeapCorrespondence CanonicalForms.
Require Import HeapUpdate.

(** * Heap Correspondence is Preserved Under Updates *)
Lemma defs_update_typing: forall Gamma hds U hds' a x T,
...
Lemma heap_update_inert: forall Gamma Sigma Sigma' x y a T,
    inert Gamma ->
...
Lemma heap_updateable: forall Sigma x U hds a o y,
...
Lemma heap_update_exists: forall Gamma Sigma x y a S T,
    inert Gamma ->
    heap_correspond Gamma Sigma ->
    Gamma ⊢ trm_var (avar_f x) ∶ typ_rcd (dec_trm a S T) ->
    Gamma ⊢ trm_var (avar_f y) ∶ S ->
    exists Sigma', heap_update Sigma x a y Sigma'.
Proof.
