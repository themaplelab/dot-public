...
Require Import Effects InitVar.
...
Fixpoint env_same_init_typ (vs : list var) (i : init_typ) : (env init_typ) :=
  vs ~** (List.map (fun _ => i) vs).
...
(** The following is an extension rule that is not part of the base calculus. *)

(** [Gamma; (Delta [ vs -> committed]) ⊢c l ]                            #<br>#
    [Gamma,x:T ; (Delta | vs ^ free),x:free ⊢i u : i ]     #<br>#
    [x fresh ]     #<br>#
    [―――――――――――――――――――――――――――――――――――――――――――――――] #<br>#
    [Gamma; (Delta | vs ^ free) ⊢i let (x:T) = l in u : i]     *)
| init_lit_free : forall L Gamma Delta vs T l u i,
    Gamma ⸴ subst_env_fset_vars Delta vs committed  ⊢c l ->
    (forall x,
        x \notin L ->
        Gamma & x ~ T ⸴ Delta & x ~ free ⸴ vs \u \{x} ⊢i open x u ∶ i) ->
...
(** [Gamma; Delta ⊢c k ]                            #<br>#
    ...
    [――――――――――――――――――――――――――――――――――――――] #<br>#
    [Gamma; (Delta | vs ^ free) ⊢i new k(y1,...) : free]     *)
| init_new_free : forall Gamma Delta vs x avs Ts is' T,
    Gamma ⸴ Delta ⊢c trm_var (avar_f x) ->
    Gamma ⊢ trm_var (avar_f x) ∶ typ_con Ts is' T ->
    Gamma ⸴ Delta ⸴ vs ⊢i avs :: is' ->
    Gamma ⸴ Delta ⸴ vs ⊢i trm_new (avar_f x) avs ∶ free
...
(** [Gamma; (Delta | vs ^ free) ⊢i x : free ]     #<br>#
    [Gamma; (Delta | vs ^ free) ⊢i y : free ]     #<br>#
    [――――――――――――――――――――――――――――――――――――――] #<br>#
    [Gamma; (Delta | vs ^ free) ⊢i x.a := y : free]     *)
| init_fld_asn_ff : forall Gamma Delta vs x y a,
    Gamma ⸴ Delta ⸴ vs ⊢i trm_var (avar_f x) ∶ free ->
    Gamma ⸴ Delta ⸴ vs ⊢i trm_var (avar_f y) ∶ free ->
    Gamma ⸴ Delta ⸴ vs ⊢i trm_asn (avar_f x) a (avar_f y) ∶ free
...
Ltac init_fresh_constructor_extern :=
  ...
  | [ _ : _ ⸴ subst_env_fset_vars _ _ _ ⊢c _ |-
      _ ⸴ _ ⸴ _ ⊢i trm_lit _ _ _ ∶ _ ] =>
    let z := fresh "z" in
    init_apply_fresh init_lit_free as z
  | [ |- _ ⸴ _ ⸴ _ ⊢i trm_lit _ _ _ ∶ _ ] =>
    let z := fresh "z" in
