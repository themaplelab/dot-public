...
Proof.
  ...
  assert (x \in dom Delta) as x_in_Delta.
  { inversions x_init.
    - rename H3 into x_init_var. inversions x_init_var; eapply binds_to_dom; eauto.
    - inversions H. rename H3 into x_init_var.
      inversions x_init_var; eapply binds_to_dom; eauto. }
  ...
  - eapply init_binds_sub_subst_cond_push with (vs:=vs).
    inversions x_init; auto.
    + econstructor. inversions H; auto.
Qed.
...
Proof.
  ...
  { intro; subst; apply HxD.
    inversion Hinit; subst; eauto using binds_to_dom. }
...
Proof.
  ...
  - apply init_var_weaken_vars.
    ...
    + inversions H8; eapply binds_to_dom; eauto.
Qed.
...
Lemma renaming_init_let : forall Gamma Delta vs T L x t i1 i2,
    ok Gamma ->
    x # Gamma ->
    x # Delta ->
    x \notin vs ->
    (forall x : var,
        x \notin L ->
        Gamma & x ~ T ⸴ Delta & x ~ i1 ⸴ vs \u \{ x} ⊢i open x t ∶ i2) ->
    Gamma & x ~ T ⸴ Delta & x ~ i1 ⸴ vs \u \{ x} ⊢i open x t ∶ i2.
Proof.
  intros * Hok Hfr HfrD Hfrvs HfrOpen.
  pick_fresh_gen ((fv t) \u (fv T) \u L \u (fv_env Gamma) \u (dom Gamma)
                         \u (dom Delta) \u vs \u \{ x}) y.
  assert (y \notin L) as HyOpen by auto.
  apply HfrOpen in HyOpen.
  apply (init_weaken (x:=x)) with (T':=T) (i':=i1) (vs':=\{x}) in HyOpen; auto.
  apply (well_init_subst_term) with (y:=x) in HyOpen; auto.
  + rewrite map_single in HyOpen.
    rewrite subst_fresh in HyOpen by auto.
    rewrite ? vars_subst_union in HyOpen.
    rewrite <- ? vars_subst_singleton_same in HyOpen.
    rewrite ? vars_subst_notin with (x:=y) in HyOpen by auto.
    rewrite <- union_assoc, union_same in HyOpen by auto.
    rewrite <- subst_intro in HyOpen by auto.
    auto.
  + rewrite map_single, subst_fresh by auto; auto.
  + assert (x <> y) as Hneq by auto.
    intros i' Hbi'. exists i1.
    split; auto.
    apply binds_push_neq_inv in Hbi'; auto.
    apply binds_push_eq_inv in Hbi'; subst; auto.
Qed.
...
(** Well-init let_lit *)
Lemma renaming_init_lit_init : forall Gamma Delta vs T L x t i,
    ok Gamma ->
    x # Gamma ->
    x # Delta ->
    (forall x : var,
...
Proof.
  ...
  assert (y \notin L) as HyOpen by auto.
  apply HfrOpen in HyOpen.
  apply (init_weaken_same_vars (x:=x)) with (T':=T) (i':=committed)
    in HyOpen; auto.
  apply (well_init_subst_term) with (y:=x) in HyOpen; auto.
  + rewrite map_single in HyOpen.
    rewrite subst_fresh in HyOpen by auto.
...
Lemma renaming_init_lit_init_comm : forall Gamma Delta T L x t,
    ok Gamma ->
    x # Gamma ->
    x # Delta ->
    (forall x : var, x \notin L -> Gamma & x ~ T ⸴ Delta & x ~ committed ⊢c open x t) ->
    Gamma & x ~ T ⸴ Delta & x ~ committed ⊢c open x t.
Proof.
  intros * Hok Hfr HfrD HfrOpen.
  pick_fresh_gen ((fv t) \u (fv T) \u L \u (fv_env Gamma) \u (dom Gamma)
  ...
  assert (y \notin L) as HyOpen by auto.
  apply HfrOpen in HyOpen.
  apply (commit_weaken (x:=x)) with (T':=T) (i':=committed) in HyOpen; auto.
  apply (well_comm_subst_term) with (y:=x) in HyOpen; auto.
  + rewrite map_single in HyOpen.
    rewrite subst_fresh in HyOpen by auto.
    rewrite <- subst_intro in HyOpen by auto.
...
(** Well-init let_lit *)
(* This lemma does not make sense in the extended calculus,
   use renaming_init_lit_init directly,
   or use renaming_init_let *)
