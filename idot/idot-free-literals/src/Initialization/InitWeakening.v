...
Require Import InitVar InitTyping.
...
Proof.
  ...
  - init_fresh_constructor_extern; eauto. assert (z \notin L) by auto.
    specialize (H0 z H1 Gamma1 x T' (Gamma2 & z ~ T));
  ...
  - init_fresh_constructor_extern; eauto. assert (z \notin L) by auto.
    ...
    auto.
  - init_fresh_constructor_extern; eauto. assert (z \notin L) by auto.
    specialize (H z H0 Gamma1 x T' (Gamma2 & z ~ T));
      repeat rewrite concat_assoc in *.
    auto.
...
Proof.
  ...
  - init_fresh_constructor_extern; eauto.
    + repeat(rewrite subst_env_fset_vars_distrib_env).
      ...
      cases_if;
        apply H; eauto using subst_env_fset_vars_distrib_env,
                 subst_env_fset_vars_dom;
        erewrite subst_env_fset_vars_dom; eauto.
    + assert (z \notin L) by auto.
      specialize (H0 z H1 Delta1 x i' (Delta2 & z ~ free));
        repeat rewrite concat_assoc in *.
      auto.
  - init_fresh_constructor_extern; eauto. assert (z \notin L) by auto.
    specialize (H0 z H1 Delta1 x i'0 (Delta2 & z ~ i));
      repeat rewrite concat_assoc in *.
    auto.
  - init_fresh_constructor_extern; eauto. assert (z \notin L) by auto.
    specialize (H0 z H1 Delta1 x i' (Delta2 & z ~ committed));
      repeat rewrite concat_assoc in *.
    auto.
Qed.
...
Proof.
  ...
  - init_fresh_constructor_extern; eauto.
    + eapply H; eauto.
      intros. eapply subst_env_fset_vars_more_E; eauto.
    + apply H0; auto. intros.
      apply binds_push_inv in H2; destruct_ors; destruct_ands; subst; auto.
  - init_fresh_constructor_extern; eauto.
    apply H0; auto. intros.
...
Lemma well_init_more_committed_rules :
  ...
  (forall Gamma Delta vs t i,
      ...
      forall Delta',
        (forall x, binds x committed Delta -> binds x committed Delta') ->
        (forall x init, x \in vs -> binds x init Delta -> binds x init Delta') ->
        Gamma ⸴ Delta' ⸴ vs ⊢i t ∶ i) /\
  (forall Gamma Delta vs avs is',
      Gamma ⸴ Delta ⸴ vs ⊢i avs :: is' ->
      forall Delta',
...
Proof.
  ...
  - init_fresh_constructor_extern; eauto.
    ...
    + intros. rewrite <- concat_assoc in H3 |- *. apply binds_concat_inv in H3.
      destruct H3 as [? | [? ?]]; auto.
    + intros. rewrite <- concat_assoc in H4 |- *. apply binds_concat_right.
  ...
  - init_fresh_constructor_extern; eauto.
    + apply H. intros. eapply subst_env_fset_vars_more_env; eauto.
    + apply H0; intros; auto.
      apply binds_push_inv in H3; destruct_ors; destruct_ands; subst; auto.
      * inversions H4.
      * apply binds_concat_inv in H4; destruct H4; eauto.
        destruct H4. apply binds_push_fresh; auto.
        rewrite in_union, in_singleton in H3.
        rewrite dom_single, notin_singleton in H4.
        destruct H3; eauto. exfalso; eauto.
  - init_fresh_constructor_extern; eauto.
    apply H0; intros; auto.
    apply binds_push_inv in H3; destruct_ors; destruct_ands; subst; auto.
    rewrite in_union_singleton_r in H3; destruct H3; subst.
    + apply binds_push_fresh; auto. apply binds_concat_left_inv in H4; auto.
      simpl_dom; apply notin_singleton. assert (z \notin vs) by auto.
      intro contra; subst; eauto.
    + apply binds_push_eq_inv in H4; subst; auto.
  - init_fresh_constructor_extern; eauto.
    apply H0; intros; auto.
    apply binds_push_inv in H3; destruct_ors; destruct_ands; subst; auto.
    apply binds_push_fresh; auto. apply binds_concat_left_inv in H4; auto.
...
Proof.
  ...
  - init_fresh_constructor_extern.
    + pose proof well_init_more_committed_rules as [_ [_ [? _]]];
        eauto using subst_env_fset_vars_more, subset_union_weak_l,
        subset_union_weak_r.
    + rewrite <- union_assoc, (union_comm vs' \{z}), union_assoc. eauto.
  - init_fresh_constructor_extern; eauto.
    rewrite <- union_assoc, (union_comm vs' \{z}), union_assoc. eauto.
Qed.
...
Lemma well_init_more_committed_renaming : forall L Gamma Delta vs S i t i' Delta',
  (forall x : var, x \notin L -> Gamma & x ~ S ⸴ Delta & x ~ i ⸴ vs \u \{ x} ⊢i open x t ∶ i') ->
  (forall x, binds x committed Delta -> binds x committed Delta') ->
  (forall x init, x \in vs -> binds x init Delta -> binds x init Delta') ->
...
Proof.
  ...
  - init_fresh_constructor_extern.
    + repeat(rewrite subst_env_fset_vars_distrib_env).
      rewrite subst_env_fset_vars_single. cases_if; eauto.
      * repeat(rewrite subst_env_fset_vars_distrib_env in c).
        rewrite subst_env_fset_vars_single in c. cases_if; eauto.
      * apply H; eauto; try(rewrite subst_env_fset_vars_dom; eauto).
        repeat(rewrite subst_env_fset_vars_distrib_env).
        rewrite subst_env_fset_vars_single. cases_if. eauto.
    + rewrite <- (concat_assoc (Delta1 & _)).
      eapply H0; eauto.
