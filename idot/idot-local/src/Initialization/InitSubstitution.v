...
Local Hint Resolve init_var_substitution_middle_empty
      init_var_substitution_middle_remove : core.
...
Local Ltac spec_ty_subst :=
    ...
    | [Hfr : ?x # ?G2, Htyt : ?G1 & ?x ~ ?T & ?G2 ⊢ ?t ∶ ?U,
       Hni : ?x \notin (fv_env ?G1), Htyy : ?G1 & (subst_env ?x ?y) ?G2
               ⊢ trm_var (avar_f ?y) ∶ (?T /[ ?x -> ?y]) |- _ ] =>
      let H := fresh "H" in
...
Lemma well_init_subst_avars : forall Gamma1 x T' Gamma2 Delta1 i' Delta2 vs avs is' y,
    ...
    Gamma1 & (subst_env x y) Gamma2 ⸴ Delta1 & Delta2⸴ (vs /[ x -> y]) ⊢i
                                                     (avs /[ x -> y]) :: is'.
Proof.
  intros * Hfr Hwi Hxy.
  ...
  - apply init_commit. inversions H0.
    ...
    * binds_middle_subst.
      ...
      apply Hxy in H3. dependent induction H3; eauto using init_var_sub_comm.
...
Proof.
  ...
  - cases_if; auto.
    + constructor.
      ...
      apply init_var_sub_comm in Hsub. subst. auto.
...
Proof with eauto.
  ...
  - dependent induction H3; eauto;
    match goal with
      | [ Hinit : ?G ⸴ ?D ⸴ ?vs \u \{ ?x } ⊢i ?t ∶ ?i ,
          Hbincom : binds ?x committed ?D
          |- ?G ⸴ ?D ⸴ ?vs ⊢i ?t ∶ ?i ] => eapply init_strengethen_vs_commit; eauto
      | [ Hinit : ?G ⸴ ?D ⸴ ?vs \u \{ ?x } ⊢i ?t ∶ ?i ,
          Hin : ?x \in ?vs
          |- ?G ⸴ ?D ⸴ ?vs ⊢i ?t ∶ ?i ] => rewrite member_union_same in Hinit; eauto
      | _ => idtac
      end.
  - inversions H.
