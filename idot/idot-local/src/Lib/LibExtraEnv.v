...
Lemma binds_single_weaken : forall (A : Type) (x x' : var) (a a' : A) (E G : env A),
    binds x a (E & G) -> x' <> x -> binds x a (E & x' ~ a' & G).
Proof.
  introv Hbix Hneq.
  apply binds_concat_inv in Hbix. destruct Hbix as [HbixG | [HxninG HbixE]]; auto.
Qed.

Lemma from_list_dom : forall A (ctx : env A) x a,
    ((x, a) \in from_list ctx) ->
    (x \in dom ctx).
Proof.
  induction ctx using env_ind; introv Hin.
  - exfalso. rewrite empty_def, from_list_nil, in_empty in Hin. auto.
  - rename x into x', x0 into x, v into a'.
    rewrite concat_def, single_def, liblist_app in Hin. simpl in Hin.
    rewrite from_list_cons, in_union, in_singleton in Hin.
    rewrite dom_push, in_union, in_singleton.
