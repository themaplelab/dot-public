...
Lemma subset_trans : forall (A : Type) (s1 s2 s3 : fset A),
    s1 \c s2 -> s2 \c s3 -> s1 \c s3.
Proof. introv Hsub12 Hsub23. hnf in *. hnf. eauto. Qed.
Hint Resolve subset_trans : core.

Lemma from_list_subset_nil_inv : forall (A : Type) (e : list A),
    from_list e \c from_list nil ->
    e = nil.
Proof.
  introv Hsub; destruct e; eauto.
  rewrite from_list_nil, from_list_cons in *. hnf in Hsub.
  specialize (Hsub a). rewrite in_empty in *. exfalso.
  apply Hsub. rewrite in_union, in_singleton. auto.
Qed.

Lemma from_list_nil_subset : forall (A : Type) (e : list A),
    from_list nil \c from_list e.
Proof. intros. rewrite from_list_nil. apply subset_empty_l. Qed.
