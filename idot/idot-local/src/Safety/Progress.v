...
Proof.
  ...
  inversions Hit.
  { (* reading from committed *)
    rename H1 into HinitSel.
    ...
    destruct Hcases as [Contra| [y [Hhas Hty]]];
      ...
      | eauto]. }
  { (* reading from local *)
    rename H7 into HinitSel.
    inversions HinitSel. rename H5 into HinitObj.
    dependent induction HinitObj.
    - rename H1 into HinE. rename H2 into HbixD.
      destruct (free_sub_heaps_well_localised Hfshs)
        as [HdomEqGD [HdomEqDS Hlhi]].
      pose proof (canonical_forms_obj Hin Hhc H0)
        as [? [hds [HbixS Hcases]]].
      pose proof (Hlhi _ _ HbixD HbixS) as Hlho.
      inversions Hlho.
      + rename H1 into Hchi. inversions Hchi. rename H4 into Hchds.
        destruct Hcases as [Contra| [y [Hhas Hty]]];
          [exfalso;
           eauto using committed_heap_defs_labels_has_None_inv
          | eauto].
      + rename H4 into Hnnhds.
        destruct Hcases as [Contra| [y [Hhas Hty]]];
          [exfalso | eauto].
        eapply non_null_heap_defs_no_null; eauto.
    - apply init_var_sub_local in H1; subst. eauto. }
Qed.
